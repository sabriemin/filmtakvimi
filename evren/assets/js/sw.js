  ...
  e.waitUntil(
    (async () => {
      ...
      try {
        ...
        /* offline install may fail; ignore */
      }
      self.skipWaiting();
    })(),
  );
...
self.addEventListener("activate", (e) => {
  e.waitUntil(
    (async () => {
      ...
      await Promise.all(
        keys.filter((k) => ![APP_CACHE, IMG_CACHE].includes(k)).map((k) => caches.delete(k)),
      );
      self.clients.claim();
    })(),
  );
